...
const debug = require('debug')('TuyaAccessoryOld');
const TuyaLightBulb = require('./TuyaLightBulbOld');
const TuyaOutlet = require('./TuyaOutletOld');
const TuyaGeneric = require('./TuyaGenericOld');
...
class TuyaAccessory {
  ...
  addDevice(log, config, homebridge) {
    ...
    if (tuyaDevice.isColor) {
      tuyaDevice.onPropertyDelay = 350;
    }
  ...
  */
  ...
  async getSchemaRetryable(deviceId) {
    ...
    // eslint-disable-next-line new-cap
    const retryable = new async.retryable({ times: 6, interval }, (callback) => {
