...
class TuyaOutlet {
  constructor(tuyaAccessory, config, homebridge) {
    ...
    this.dpsVolts = 6;
    if (config.logErrors) {
      logError = this.tuyaDev.log ? this.tuyaDev.log.error : debug;
    } else {
      logError = debug;
    }
  ...
  calculateTotalConsumption(callback) {
    let lastTime;
    let totalConsumption = 0;
    const countTotalConsumption = (history) => {
    ...
    fs.readFile(filename, 'utf8', (error, data) => {
      if (!error && data) {
        ...
        countTotalConsumption(jsonFile.history);
      } else if (error) {
  ...
  getInUse(callback = () => {}) {
    this.tuya.getProperties(this.devId, [this.dpsOnOff, this.dpsAmperes])
      ...
      .catch((error) => {
        logError(this.name, 'getting in use status error', error.message);
        callback(error);
      });
  }
  ...
  getCurrent(callback = () => {}) {
    this.tuya.getProperty(this.devId, this.dpsAmperes)
      ...
      .catch((error) => {
        logError(this.name, 'getting current error', error.message);
        callback(error);
      });
  }
  ...
  getConsumption(callback = () => {}) {
    this.tuya.getProperty(this.devId, this.dpsWatts)
      ...
      .catch((error) => {
        logError(this.name, 'getting consumption error', error.message);
        callback(error);
      });
  }
  ...
  getVoltage(callback = () => {}) {
    this.tuya.getProperty(this.devId, this.dpsVolts)
      ...
      .catch((error) => {
        logError(this.name, 'getting voltage error', error.message);
        callback(error);
      });
  }
  ...
  getVA(callback = () => {}) {
    this.tuya.getProperties(this.devId, [this.dpsAmperes, this.dpsVolts])
      ...
      .catch((error) => {
        logError(this.name, 'getting volto-amperes error', error.message);
        callback(error);
      });
  }
}
