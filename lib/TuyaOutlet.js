...
class TuyaOutlet extends TuyaAccessory {
    ...
    error(...args) {
        if (this.config.logErrors) {
            this.hblog.error('Error:', ...args, '[TO]');
        } else {
            ...
            if (this.interval) {
                ...
                const updateValues = () => setTimeout(async () => {
                    ...
                    try {
                        ...
                        if (watts !== undefined && !isNaN(watts)) {
                        ...
                        if (!extra) {
                            historyService.setExtraPersistedData({ totalConsumption, resetTotal });
    ...
    readTotalConsumption() {
        try {
            ...
            const totalConsumption = jsonData.extra && jsonData.extra.totalConsumption ? jsonData.extra.totalConsumption : 0.0;
            ...
            this.debug('readTotalConsumption error:', err);
            return [0.0, 0];
        }
    }
