...
class TuyaAccessory {
  ...
  addDevice(log, config, homebridge) {
    ...
    if (tuyaDevice.isColor) {
      tuyaDevice.onPropertyDelay = 100;
    }
  ...
  */
  ...
  async resolveIdNowRetryable(deviceId) {
    ...
    const retryable = new async.retryable({ times: 3, interval }, (devId, callback) => {
      ...
      this.getDevTuya(devId).find()
        ...
        .catch((error) => {
          this.log(devId, this.getDevName(devId), 'find IP error', error);
          callback(error);
        });
    });
    return new Promise((resolve, reject) => {
  ...
  async resolveIdNow(deviceId) {
    return new Promise((resolve, reject) => {
      ...
      this.getDevTuya(deviceId).find()
        .then(() => {
          try {
            this.getDevTuya(deviceId).connect()
              .then(() => {
                this.log(deviceId, `Resolve & connect ${this.getDevName(deviceId)} IP: ${this.getDevTuya(deviceId).device.ip}`);
        ...
        .catch((error) => {
          this.log(deviceId, this.getDevName(deviceId), 'find IP error', error);
          reject(error);
        });
    });
  }
  ...
  async getHandle(deviceId) {
    return new Promise((resolve, reject) => {
      ...
      if (this.getDevTuya(deviceId) && this.getDevTuya(deviceId).isConnected()) {
        resolve(this.getDevTuya(deviceId));
  ...
  async getSchemaRetryable(deviceId) {
    ...
    // eslint-disable-next-line new-cap
    const retryable = new async.retryable({ times: 3, interval }, (callback) => {
