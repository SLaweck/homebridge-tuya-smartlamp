...
class TuyaAccessory {
  ...
  addDevice(log, device, homebridge) {
    ...
    if (tuyaDevice.isColor) {
      tuyaDevice.setPropertyDelay = 200;
    }
  ...
  async resolveId(deviceId) {
    return new Promise((resolve, reject) => {
      debug('resolveId', this.getDevName(deviceId));
  ...
  async resolveIdNowRetryable(deviceId) {
    const interval = (retryCount) => {
      debug('interval in retryable in resolveIdNowRetryable', this.getDevName(deviceId), 'retryCount', retryCount);
      return 100 * retryCount;
    };
    // eslint-disable-next-line new-cap
    const retryable = new async.retryable({ times: 25, interval }, (devId, callback) => {
      debug('retryable in resolveIdNowRetryable', this.getDevName(deviceId));
    ...
    return new Promise((resolve, reject) => {
      debug('resolveIdNowRetryable', this.getDevName(deviceId));
  ...
  async resolveIdNow(deviceId) {
    return new Promise((resolve, reject) => {
      debug('resolveIdNow', this.getDevName(deviceId));
  ...
  async getHandle(deviceId) {
    return new Promise((resolve, reject) => {
      debug('getHandle', this.getDevName(deviceId));
  ...
  async getSchemaRetryable(deviceId) {
    const interval = (retryCount) => {
      debug('interval in retryable in getSchemaRetryable', this.getDevName(deviceId), 'retryCount', retryCount);
      if (retryCount > 3) {
    ...
    // eslint-disable-next-line new-cap
    const retryable = new async.retryable({ times: 25, interval }, (callback) => {
      debug('retryable in getSchemaRetryable', this.getDevName(deviceId));
    ...
    return new Promise((resolve, reject) => {
      debug('getSchemaRetryable', this.getDevName(deviceId));
  ...
  async getSchema(deviceId) {
    return new Promise((resolve, reject) => {
      debug('getSchema', this.getDevName(deviceId));
  ...
  async getProperty(deviceId, index) {
    return new Promise((resolve, reject) => {
      debug('getProperty', this.getDevName(deviceId), index);
  ...
  async getProperties(deviceId, indexes) {
    return new Promise((resolve, reject) => {
      debug('getProperty', this.getDevName(deviceId), indexes);
      this.getSchema(deviceId)
        .then((props) => {
          let propMap;
          try {
  ...
  async setProperty(deviceId, index, newValue) {
    return new Promise((resolve, reject) => {
      debug('setProperty', this.getDevName(deviceId), index, newValue);
      setTimeout(() => {
      ...
      }, this.getDev(deviceId).setPropertyDelay);
    });
  }

  async setPropertyNow(deviceId, index, newValue) {
    return new Promise((resolve, reject) => {
      debug('setPropertyNow', this.getDevName(deviceId), index, newValue);
      this.getHandle(deviceId)
  ...
  async setPropertyDebounce(deviceId, index, newValue) {
    debug('setPropertyDebounce', this.getDevName(deviceId), index, newValue);
  ...
  nextSetProperty(deviceId, index, newValue) {
    ...
    debug('nextSetProperty', this.getDevName(deviceId), setProp);
    let setPropSubject = this.getDev(deviceId).setPropSubjects[index];
    if (!setPropSubject) {
