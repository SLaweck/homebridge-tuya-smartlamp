...
class TuyaAccessory {
  ...
  async resolveId(deviceId) {
    return new Promise((resolve, reject) => {
      debug('resolveQueue push', deviceId);
  ...
  async getSchemaRetryable(deviceId) {
    // eslint-disable-next-line new-cap
    const retryable = new async.retryable({ times: 25, interval: retryCount => 100 * retryCount }, (callback) => {
      this.getHandle(deviceId)
        .then((handle) => {
          handle.get({ schema: true })
            ...
            .catch((error) => {
              this.log(deviceId, this.getDevName(deviceId), 'get schema error', error.message);
    ...
    return new Promise((resolve, reject) => {
      setTimeout(() => {
        this.updateQueue.push((callback) => {
          debug('call setPropertyNow', deviceId, index, newValue);
