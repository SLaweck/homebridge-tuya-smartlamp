...
class TuyaAccessory {
  ...
  log(deviceId, ...args) {
    return this.devices[deviceId].log(...args);
  }
  ...
  async getProperties(deviceId, indexes) {
    return new Promise((resolve, reject) => {
      this.getSchema(deviceId)
        .then(dps => indexes.map(key => dps[key]))
        .catch(error => reject(error));
    });
  }
  ...
  async getOnOff(deviceId) {
    return new Promise((resolve, reject) => {
      this.getProperty(deviceId, this.getOnOffDps(deviceId))
        ...
        .catch((error) => {
          this.log(deviceId, 'getting device on/off error', error);
          reject(error);
        });
    });
  }
  ...
  async setOnOff(deviceId, onOff) {
    return new Promise((resolve, reject) => {
      this.setProperty(deviceId, this.getOnOffDps(deviceId), onOff)
        ...
        .catch((error) => {
          this.log(deviceId, 'setting device on/off error', error);
          reject(error);
        });
    });
  }
    ...
    return new Promise((resolve, reject) => {
      ...
      if (dps) {
        this.setProperty(deviceId, dps, bright)
          ...
          .catch((error) => {
            this.log(deviceId, 'setting device bright error', error);
            reject(error);
          });
      } else {
        resolve(-1);
