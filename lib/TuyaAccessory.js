  ...
  */
      ...
      this.getDevTuya(devId).find()
        .then(() => {
          this.getDevTuya(devId).connect()
            ...
            .catch((error) => {
              this.log(devId, this.getDevName(devId), 'connect error');
              callback(error);
            });
        })
        .catch((error) => {
      ...
      this.getDevTuya(deviceId).find()
        .then(() => {
          this.getDevTuya(deviceId).connect()
            ...
            .catch((error) => {
              this.log(deviceId, this.getDevName(deviceId), 'connect error');
              reject(error);
            });
        })
        .catch((error) => {
          this.log(deviceId, this.getDevName(deviceId), 'resolve IP error');
          reject(error);
        });
    });
  }
  ...
  async getHandle(deviceId) {
    return new Promise((resolve, reject) => {
      ...
      if (this.getDevTuya(deviceId)._connected) {
        resolve(this.getDevTuya(deviceId));
