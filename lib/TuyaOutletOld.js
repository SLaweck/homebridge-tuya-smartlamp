...
const debug = require('debug')('TuyaOutletOld');
...
class TuyaOutlet {
  ...
  calculateTotalConsumption(callback) {
    ...
    const countTotalConsumption = (json) => {
      ...
      if (extra && extra.totalConsumption > 0) {
        debug('get totalConsumption from extra', extra);
        ({ totalConsumption } = extra);
      } else {
        debug('calcultae totalConsumption from history', firstEntry, lastEntry, usedMemory, lastTime, extra);
        for (let i = firstEntry; i < lastEntry; i++) {
          ...
          if (typeof entry === 'object') {
            if (entry.time && entry.power !== undefined) {
              ...
              if (Number.isNaN(consumption / kHour) || delta < 0) {
                debug('history totalConsumption', totalConsumption, now, watts, delta, consumption, consumption / kHour, i);
              } else {
                totalConsumption += consumption / kHour;
              }
              lastTime = now;
